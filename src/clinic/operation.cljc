(ns clinic.operation
  "OperationActor -- one medical/dental-practice operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (ClinicOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Clinical Practice Governor
  ...
  One graph run = one medical/dental-practice operation (intake ->
  ...
  decision to a human operator (the licensed physician/dentist). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:treatment/administer` ALWAYS reaches this node when the
  governor is clean -- see `clinic.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [clinic.clinicopsllm :as clinicopsllm]
            [clinic.governor :as governor]
...
(defn build
  ...
  `clinic.store/Store`).
  opts:
    :advisor      -- a `clinic.clinicopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (clinicopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ClinicOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (clinicopsllm/-advise advisor store request)]
            {:proposal p :audit [(clinicopsllm/trace request p)]})))

      ;; Clinical Practice Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
